...
TNode::TNode(SurfReconstructor* surf_constructor, std::shared_ptr<TNode> parent, Index i)
{
	...
	center = parent->center + (cstoneOctree::Vec3f(sign(i.x), sign(i.y), sign(i.z)) * half_length);
	//node[0] = center[0];
	//node[1] = center[1];
	//node[2] = center[2];
	children[0] = children[1] = children[2] = children[3] =
	...
	float* sample_points)
{
	...
	cstoneOctree::Vec3f minV(center[0] - half_length, center[1] - half_length, center[2] - half_length);
	cstoneOctree::Vec3f maxV(center[0] + half_length, center[1] + half_length, center[2] + half_length);
...
//	cstoneOctree::Vec3f minV(center[0] - half_length + border, center[1] - half_length + border, center[2] - half_length + border);
//	cstoneOctree::Vec3f maxV(center[0] + half_length - border, center[1] + half_length - border, center[2] + half_length - border);
//	QEFNormal<float, 4> node_q;
//	node_q.zero();
//	std::vector<cstoneOctree::Vec3f> node_plane_norms, node_plane_pts;
//	int node_index;
//	for (int z = 0; z <= constructor->getOverSampleQEF(); z++)
//	{
//		for (int y = 0; y <= constructor->getOverSampleQEF(); y++)
