...
bool VisitorExtract::on_node_iso(TraversalData &td)
{
	if (td.n->points != nullptr)
	{
		int index = 0;
		for (int i = 0; i < 8; i++)
		{
			if (td.n->vertvs[i][3] > 0)
			{
				index += 1 << i;
			}
		}
		...
		std::array<Eigen::Vector4f, 8> trans_vertices;
		for (int i = 0; i < 8; i++)
		{
			trans_vertices[i] = td.n->vertvs[proc.trans[i]];
		}
		...
		auto calculate_point = [&](int e_index, int v_index1, int v_index2) {
			...
			float ratio = invlerp(v1[3], v2[3], 0.0f);
			if (ratio < constructor->getRatioTolerance())
				td.n->points->at(e_index) = v1.head(3);
			else if (ratio > (1 - constructor->getRatioTolerance()))
				td.n->points->at(e_index) = v2.head(3);
			else
				td.n->points->at(e_index) = lerp(v1, v2, ratio).head(3);
		};
		...
		auto append = [&](int index1, int index2, int index3) {
			const auto& p1 = td.n->points->at(index1);
			const auto& p2 = td.n->points->at(index2);
			const auto& p3 = td.n->points->at(index3);
			int pIdx1 = m->insert_vert(p1);
			int pIdx2 = m->insert_vert(p2);
			int pIdx3 = m->insert_vert(p3);
			if (proc.flip)
...
bool VisitorExtract::on_face_iso(TraversalData &td0, TraversalData &td1, char orient)
{
	if (td0.n->points != nullptr && td1.n->points != nullptr)
	{
