...
UniformGrid::UniformGrid(const std::vector<Vec3f> &particles, const std::vector<float> &radiuses)
{
...
UniformGrid::UniformGrid(const std::vector<Vec3f> &particles, float radius)
{
    _GlobalParticles = particles;
	_GlobalParticlesNum = _GlobalParticles.size();
	_RADIUS = radius;
...
void UniformGrid::loadRootBox()
{
	...
	for (const Vec3f& p: _GlobalParticles)
	{
...
{
    Vec3f minV(_BoundingBox[0], _BoundingBox[2], _BoundingBox[4]);
    Vec3f maxV(_BoundingBox[1], _BoundingBox[3], _BoundingBox[5]);
...
void UniformGrid::Run(float iso_value, std::string filename, std::string filepath) 
{
    ...
    _searcher = std::make_shared<MultiLevelSearcher>(&_GlobalParticles, _BoundingBox, &_GlobalRadiuses, 4.0f, 1.5f);
