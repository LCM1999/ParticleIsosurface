...
*/
KDTreeNeighborhoodSearcher::KDTreeNeighborhoodSearcher(std::vector<Eigen::Vector3f>* _particles, std::vector<double>* _radius, std::vector<int>* _index)
{
    index_map.clear();
    if (_index == nullptr)
    {
...
void KDTreeNeighborhoodSearcher::InitializeGroups()
{
    ...
    int group_size = int(sqrt(particles.size())); // 这里设定分组大小为 sqrt(n)，可按需更改
...
void KDTreeNeighborhoodSearcher::GetNeighborhood(Eigen::Vector3f target, double radius, std::vector<int>* neighborhood_ids, std::vector<Eigen::Vector3f>* neighborhood_coordinates)
{
    ...
    for (int i = 0; i < all_group_ids.size(); ++i)
    {
        ...
        if ((particles[cur_id].coordinate - target).norm() <= particles[cur_id].radius + radius)
        {
            if (neighborhood_ids != nullptr)
