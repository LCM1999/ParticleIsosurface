...
public:
	...
	void SingleEval(const Eigen::Vector3f& pos, float& scalar, Eigen::Vector3f& gradient, bool use_normalize, bool use_signed);
...
inline Evaluator::Evaluator(
		std::vector<Eigen::Vector3f>* global_particles, std::vector<float>* global_density, std::vector<float>* global_mass)
{
	this->GlobalPoses = global_particles;
	GlobalDensity = global_density;
	GlobalMass = global_mass;

    GlobalxMeans = new Eigen::Vector3f[GlobalParticlesNum];
    GlobalGs = new Eigen::Matrix3f[GlobalParticlesNum];
    //compute_Gs_xMeans();
...
inline void Evaluator::SingleEval(const Eigen::Vector3f& pos, float& scalar, Eigen::Vector3f& gradient, bool use_normalize = true, bool use_signed = true)
{
	if (MAX_VALUE >= 0)
	{
		scalar = 0;
...
inline float Evaluator::AnisotropicInterpolate(const int pIdx, const Eigen::Vector3f& diff)
{
    ...
    case 0:
        ...
        break;
    case 1:
        k_value = spiky_kernel((GlobalGs[pIdx] * diff).norm(), INFLUENCE);
        break;
    case 2:
        k_value = viscosity_kernel((GlobalGs[pIdx] * diff).norm(), INFLUENCE);
        break;
    default:
        k_value = general_kernel((GlobalGs[pIdx] * diff).squaredNorm(), influnce2);
        break;
